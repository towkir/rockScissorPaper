...
var showModal = function (result) {
    ...
    modal.getElementsByTagName('h2')[0].innerHTML = result;
...
function winCheck() {
    ...
    if (playerChoosed == botChoosed) {
        ...
        }, 500); // waiting for the transition (image changing);
...
function waitBeforeWinCheck() {
    setTimeout(winCheck, 1600);
} // because the transition of the bot and player
